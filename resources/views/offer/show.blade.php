...
<div class="flex flex-row mx-10 mb-10 max-w-full flex-wrap md:mx-24 md:mb-24 md:flex-col">
    ...
    <div class="flex flex-col md:flex-row flex-wrap h-full w-full">
        ...
        <div class="flex md:flex-row flex-col items-center md:justify-around md:ml-5 md:mr-5 m-0 mt-10 w-full">
            ...
            <a
                @if(Auth::check() && ($offer->status != 'inactive' || $offer->status != 'closed'))
                    href="{{ route('offer.buy.menu', $offer) }}"
                @elseif(!Auth::check())
                    href="{{ route('login') }}"
                @else
                ...
                @endif
                type="button"
                class="h-8 w-32 md:h-12 text-white {{ $offer->status == 'inactive' || $offer->status == 'closed' ? 'bg-gray-200'  : 'bg-gray-700 hover:bg-gray-800' }} focus:ring-4 focus:outline-none focus:ring-gray-500 font-medium rounded-lg md:text-sm text-xs px-5 py-2.5 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
